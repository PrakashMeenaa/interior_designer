...
const Footer: React.FC = () => {
  ...
  useEffect(() => {
    if (logosRef.current) {
      ...
      let sibling = ul.nextSibling as HTMLElement;
  ...
  return (
    <div>
      <div className="relative mt-24 bg-cover sm:bg-left as w-screen tracking-widest leading-normal font-thin lg:bg-fixed lg:h-[500px]" style={{ backgroundImage: `url(/Images/2.jpg)` }}>
        <div className="bg-primary bg-opacity-55 h-full">
          ...
          <div className="w-screen bg-[#423329] h-max lg:relative lg:bottom-28 flex lg:pl-10">
            <div
              ...
              className="w-screen lg:w-[600px] h-36 py-5 inline-flex flex-nowrap gap-10 overflow-hidden  "
            >
              <ul ref={logosRef} className="flex items-center gap-10  justify-between animate-infinite-scroll  ">
                {[1, 2, 3, 4, 5, 6].map((index) => (
                  <div key={index} className=" aspect-square h-[100%] w-[100%]">
                    <img src={`/client/${index}.png`} alt={`${index}`} className="h-full w-full" />
                  </div>
                ))}
              </ul>
              <ul className="flex items-center gap-10 justify-between animate-infinite-scroll" aria-hidden="true">
                {[1, 2, 3, 4, 5, 6].map((index) => (
                  <div key={index} className="aspect-square h-[100%] w-[100%]">
                    <img src={`/client/${index}.png`} alt={`${index}`} className="h-full w-full" />
                  </div>
      ...
      <div className="w-screen mx-2 sm:px-16 md:px-32 lg:mt-32">
        ...
        <div className="w-full flex flex-col gap-1 text-second hover:text-white md:grid md:grid-cols-3">
          ...
          <div className="border-2 border-opacity-20 border-third w-full flex flex-col gap-2 px-3 py-10">
            <ul className="flex md:justify-center md:items-center gap-2 ">
              <li className="border-[0.5px] w-10 h-10 rounded-full flex justify-center items-center border-second text-second hover:text-white hover:border-white">
                <a href="#" className=" ">
